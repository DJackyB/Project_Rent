...
namespace BaoZuPo.Editor
{
    /// <summary>
    /// 平衡模拟器 EditorWindow。
    /// 在 Editor 中实时分析和可视化卡牌数值平衡、经济循环、贷款压力曲线。
    ///
    /// 功能：
    /// - 参数调整：起始金额、贷款金额/间隔/增长、模拟回合数
    /// - 卡牌分析：显示每张卡的成本、基础收益、净收益、ROI、回本周期
    /// - 贷款曲线：绘制贷款额随回合增长的曲线，直观显示压力
    /// - 筛选排序：按类型、稀有度、净收益等条件筛选和排序
    /// - 实时计算：修改参数后自动重新计算
    ///
    /// 打开方式：
    ...
    /// 使用场景：
    /// - 卡牌平衡调整：评估新卡牌的收益是否合理
    /// - 难度曲线设计：确保贷款压力随回合逐步增加，不会一上来压死玩家
    /// - 经济循环验证：检查是否存在容易破产的情况或无限赚钱的情况
    /// </summary>
